...
static void slaveBootupHdl(CO_Data* d, UNS8 nodeId)
{
	...
	if(tid == NULL)
	{
		printf("canopen config thread start failed!\n");
	}
}
...
static void config_servo(uint8_t nodeId)
{
	...
	LeaveMutex();
	rt_sem_take(&(servo_conf[nodeId - 2].finish_sem), osWaitForever);
}
...
static void config_servo_param_cb(CO_Data* d, UNS8 nodeId)
{
	...
	{
		conf->try_cnt++;
		printf("write SDO failed!  nodeId = %d, abortCode = 0x%08X\n", nodeId, abortCode);
		if(conf->try_cnt < 3)
		{
		...
		else
		{
			...
			conf->state = 0;
			conf->try_cnt = 0;
			printf("SDO config try count > 3, config failed!\n");
		}
	}
	else
	{
